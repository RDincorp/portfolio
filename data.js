/**
 * Portfolio Data Configuration
 * Заполняется строго по данным пользователя.
 */

const PROFILE_DATA = {
  fullName: "Радкевич Дамир Борисович",
  shortName: "Радкевич Д. Б.",
  academicTitle: "Магистр политологии, младший научный сотрудник",
  affiliation: "Центр политологии Института социологии НАН Беларуси",
  degree: "Магистр политологии (БГУ)",
  location: "Минск",
  birthDate: "17.09.2002",
  email: "rd.inskiya@gmail.com",
  phone: "+375257340078",
  
  aboutBio: "Младший научный сотрудник Центра политологии Института социологии НАН Беларуси. Выпускник магистратуры кафедры политологии юридического факультета Белорусского государственного университета (Минск). Лауреат I Республиканского молодёжного конкурса аналитических проектов (исследований) Белорусского института стратегических исследований (2024).",
  
  researchFocus: [
    "Дипломатические отношения СССР и стран Корейского полуострова",
    "Демографическая и семейная политика",
    "Социальная изоляция",
    "История международных отношений", 
    "Политическая теория и философия"
  ],

  analyticalStack: [
    { name: "Количественные и качественные методы", tools: "" },
    { name: "Дискурс-анализ и контент-анализ", tools: "" },
    { name: "Статистический и корреляционный анализ", tools: "" },
    { name: "SWOT-анализ", tools: "" },
    { name: "Наблюдение и сводка", tools: "" },
    { name: "ИИ-инструменты для расчётов", tools: "" }
  ],
  stats: {
    conferencesCount: "10",
    memosCount: "5",
    researchProjectsCount: "1 и 1 в работе",
    yearsExperience: "2+ года"
  },

  academicLinks: {
    orcid: "",
    eLibrary: "",
    researchGate: "https://www.researchgate.net/profile/Damir-Radkevich",
    institute: "https://socio.bas-net.by/damir-borisovich-radkevich/",
    telegram: "https://t.me/radkoyanka",
    email: "mailto:rd.inskiya@gmail.com"
  }
};

/**
 * Опубликованные работы
 */
const PORTFOLIO_ITEMS = [
  {
    id: "work-01",
    type: "conference",
    typeLabel: "Доклады",
    title: "ОНЛАЙН-СООБЩЕСТВА КАК ФАКТОР ФОРМИРОВАНИЯ НЕАУТЕНТИЧНОЙ ИДЕНТИЧНОСТИ",
    year: "2026",
    date: "Май 2026 г.",
    badge: "Доклады",
    badgeType: "conference",
    tags: [],
    summary: "Цифровые технологии и искусственный интеллект в обеспечении информационно-политической безопасности государств: дискурсы, концепты и практики. Белорусская политология: многообразие в единстве. К 30-летию Всебелорусского народного собрания (Минск, Том 2).",
    details: {
      eventOrContext: "Цифровые технологии и искусственный интеллект в обеспечении информационно-политической безопасности государств: дискурсы, концепты и практики. Белорусская политология: многообразие в единстве. К 30-летию Всебелорусского народного собрания · At: Минск · Volume: 2",
      url: "https://www.researchgate.net/publication/411958878_ONLAJN-SOOBSESTVA_KAK_FAKTOR_FORMIROVANIA_NEAUTENTICNOJ_IDENTICNOSTI"
    }
  },
  {
    id: "work-02",
    type: "conference",
    typeLabel: "Доклады",
    title: "Трансформация идентичности в условиях социальной атомизации",
    year: "2026",
    date: "Апрель 2026 г.",
    badge: "Доклады",
    badgeType: "conference",
    tags: [],
    summary: "Международный молодёжный научный форум «Ломоносов-2026» (Москва).",
    details: {
      eventOrContext: "Международный молодёжный научный форум «Ломоносов-2026» · At: Москва",
      url: "https://www.researchgate.net/publication/411965639_Transformacia_identicnosti_v_usloviah_socialnoj_atomizacii"
    }
  },
  {
    id: "work-03",
    type: "conference",
    typeLabel: "Доклады",
    title: "Внешнеполитическая роль сотрудников Академии наук СССР в международных дипломатических процессах эпохи «перестройки»",
    year: "2025",
    date: "Июль 2025 г.",
    badge: "Доклады",
    badgeType: "conference",
    tags: [],
    summary: "Всероссийская научная конференция (с международным участием) (Ростов-на-Дону, 4 июля 2025 г., Том III).",
    details: {
      eventOrContext: "Всероссийская научная конференция (с международным участием) (Ростов-на-Дону, 4 июля 2025 г.) · At: Ростов-на-Дону · Volume: III",
      url: "https://www.researchgate.net/publication/398814486_Vnesnepoliticeskaa_rol_sotrudnikov_Akademii_nauk_SSSR_v_mezdunarodnyh_diplomaticeskih_processah_epohi_perestrojki"
    }
  },
  {
    id: "work-04",
    type: "conference",
    typeLabel: "Доклады",
    title: "Демографический спад как вызов Союзному государству: международный опыт и научные теории противодействия",
    year: "2024",
    date: "Ноябрь 2024 г.",
    badge: "Доклады",
    badgeType: "conference",
    tags: [],
    summary: "Безопасность Союзного государства: вызовы и угрозы : материалы международной научно-практической конференции (Минск, 22 ноября 2024 г.).",
    details: {
      eventOrContext: "Безопасность Союзного государства: вызовы и угрозы : материалы международной научно-практической конференции (Минск, 22 ноября 2024) · At: Минск",
      url: "https://www.researchgate.net/publication/397195334_Demograficeskij_spad_kak_vyzov_Souznomu_gosudarstvu_mezdunarodnyj_opyt_i_naucnye_teorii_protivodejstvia"
    }
  },
  {
    id: "work-05",
    type: "article",
    typeLabel: "Статьи",
    title: "«Перестройка» на Юг: из истории установления дипломатических отношений СССР и Республики Корея",
    year: "2024",
    date: "Ноябрь 2024 г.",
    badge: "Статьи",
    badgeType: "article",
    tags: [],
    summary: "В работе раскрывается динамика советско-южнокорейских отношений в период «перестройки» с акцентом на отдельных дипломатических актах. Выявляются противоречивые воспоминания и интерпретации участников описываемых событий. Описывается непоследовательность политического курса СССР в отношениях с КНДР и РК. Приводятся примеры личных инициатив политических деятелей, прямо повлиявших на установление дипломатических связей Советского Союза и Южной Кореи.",
    details: {
      eventOrContext: "Свободная статья для ResearchGate",
      url: "https://www.researchgate.net/publication/387363675_Perestrojka_na_Ug_iz_istorii_ustanovlenia_diplomaticeskih_otnosenij_SSSR_i_Respubliki_Korea"
    }
  },
  {
    id: "work-06",
    type: "media",
    typeLabel: "ТВ и СМИ",
    title: 'БЕЛАРУСЬ И ЕАЭС | "Мнения" | 24.12.2025',
    year: "2025",
    date: "24 декабря 2025 г.",
    badge: "ТВ и СМИ",
    badgeType: "media",
    tags: [],
    summary: "Телевизионное выступление в программе «Мнения», посвящённое взаимоотношениям Республики Беларусь и Евразийского экономического союза (ЕАЭС).",
    details: {
      eventOrContext: "Телевизионная программа «Мнения» · О взаимоотношениях Республики Беларусь и Евразийского экономического союза",
      url: "https://socio.bas-net.by/o-vzaimootnosheniyah-respubliki-belarus-i-evrazijskogo-ekonomicheskogo-soyuza/"
    }
  },
  {
    id: "work-07",
    type: "current_nir",
    typeLabel: "Актуальный НИР",
    title: "НИР «Стратегии обеспечения общественно-политической устойчивости модели белорусской государственности в условиях внешних и внутренних вызовов и угроз» (2026-2030)",
    year: "2026-2030",
    date: "2026-2030 гг.",
    badge: "Актуальный НИР",
    badgeType: "current_nir",
    tags: [],
    summary: "Текущий научно-исследовательский проект Центра политологии Института социологии НАН Беларуси (2026-2030 гг.).",
    details: {
      eventOrContext: "Научно-исследовательский проект Центра политологии Института социологии НАН Беларуси (2026-2030 гг.)"
    }
  },
  {
    id: "work-08",
    type: "completed_nir",
    typeLabel: "Реализованный НИР",
    title: "НИР «Развитие и укрепление народовластия в Республике Беларусь как приоритет государственной политики» (2024-2025)",
    year: "2024-2025",
    date: "2024-2025 гг.",
    badge: "Реализованный НИР",
    badgeType: "completed_nir",
    tags: [],
    summary: "Выполненный научно-исследовательский проект Центра политологии Института социологии НАН Беларуси (2024-2025 гг.).",
    details: {
      eventOrContext: "Научно-исследовательский проект Центра политологии Института социологии НАН Беларуси (2024-2025 гг.)"
    }
  },
  {
    id: "work-09",
    type: "achievement",
    typeLabel: "Достижения",
    title: "Лауреат I Республиканского молодёжного конкурса аналитических проектов (исследований) Белорусского института стратегических исследований (2024)",
    year: "2024",
    date: "2024 г.",
    badge: "Достижения",
    badgeType: "achievement",
    tags: [],
    summary: "Тема проекта: «Использование нейронных сетей в физических рекламных баннерах: опыт Республики Беларусь».",
    details: {
      eventOrContext: "I Республиканский молодёжный конкурс аналитических проектов (исследований) Белорусского института стратегических исследований (БИСИ, 2024 г.)",
      url: "https://bisr.gov.by/provedyonnye-meropriyatiya/luchshie-molodezhnye-proekty-vybrali-v-belorusskom-institute"
    }
  },
  {
    id: "work-10",
    type: "field",
    typeLabel: "Полевой опыт",
    title: "Проведение опроса Института социологии на XXXIII-й Международной книжной выставке-ярмарке",
    year: "2026",
    date: "17.03.2026",
    badge: "Полевой опыт",
    badgeType: "field",
    tags: [],
    summary: "Проведение социального опроса общественного мнения посетителей книжной выставки. Тема опроса: читательские предпочтения посетителей",
    details: {
      eventOrContext: "Проведение опроса Института социологии на XXXIII-й Международной книжной выставке-ярмарке, пр-т Победителей, 14, г. Минск, 17.03.2026"
    }
  },
  {
    id: "work-11",
    type: "side_project",
    typeLabel: "Сторонние проекты",
    title: "Генератор библиографических ссылок по ВАК РБ",
    year: "2025",
    date: "2025 г.",
    badge: "Сторонние проекты",
    badgeType: "side_project",
    tags: [],
    summary: "Веб-инструмент для автоматизации оформления библиографических ссылок и списков литературы по государственным стандартам ВАК Республики Беларусь.",
    details: {
      eventOrContext: "Онлайн-сервис для исследователей и авторов научных публикаций по ГОСТ ВАК Республики Беларусь",
      url: "https://bibliography-vak-2025.netlify.app/"
    }
  }
];
